...
oc process -f https://raw.githubusercontent.com/openshift-labs/devops-labs/ocp-3.5/lab-7/coolstore-bluegreen-template.yaml \
...
rm -rf /tmp/pipelines && \
    ...
    pushd /tmp/pipelines && \
    curl -sL https://raw.githubusercontent.com/openshift-labs/devops-labs/ocp-3.5/lab-8/Jenkinsfile | sed "s|git url: .*|git url: '$CART_REPO'|g" > Jenkinsfile && \
    git add Jenkinsfile && \
...
curl -sL https://raw.githubusercontent.com/openshift-labs/devops-labs/ocp-3.5/lab-4/cart-pipeline-scm.yaml | sed "s|uri: .*|uri: $PIPELINES_REPO|g" | oc create -f - -n dev
